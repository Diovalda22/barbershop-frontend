...
const Wrap = styled.div`min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:url('/hero_bg.png') center/cover no-repeat;position:relative;&::before{content:'';position:absolute;inset:0;background:rgba(26,26,24,0.85);z-index:0}`
...
export function CustomerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    localStorage.setItem('customer_name', derivedName)
    ...
    <>
      ...
      <Wrap>
        <Card>
          ...
          <Sub>Silakan login untuk mengatur reservasi Anda</Sub>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <BtnGold type="submit" disabled={isLoading}>{isLoading ? 'Memproses...' : 'LOGIN'}</BtnGold>
          </Form>

          <LinkText>Belum punya akun? <Link to="/user/register">Daftar sekarang</Link></LinkText>
